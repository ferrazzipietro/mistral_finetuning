...
def main(ADAPTERS_CHECKPOINT,
  ...
  train_data = train_data.map(lambda samples: tokenizer(samples[training_params.dataset_text_field]), batched=True)
  val_data = val_data.map(lambda samples: tokenizer(samples[training_params.dataset_text_field]), batched=True)
