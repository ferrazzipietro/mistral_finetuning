...
def main(ADAPTERS_CHECKPOINT,
  ...
  if False:#not model_loading_params.quantization:
    model = AutoModelForCausalLM.from_pretrained(
      config.BASE_MODEL_CHECKPOINT, #'google/gemma-2b',
      #device_map="auto",
      cache_dir='/data/disk1/share/pferrazzi/.cache',
      torch_dtype=torch.bfloat16# model_loading_params.torch_dtype,
      )
  else:
    from accelerate import Accelerator
  ...
  training_arguments = SFTConfig(
      per_device_train_batch_size= 8, # training_params.per_device_train_batch_size,
